...
class ACVCCorruption:
    def __init__(self):
        self.acvc_generator = ACVCGenerator()

    def __call__(self, img):
        """
        img: np.ndarray, shape (H, W, C), RGB, dtype=np.uint8
        return: img after corruption
        """
        if isinstance(img, Image.Image):
            img = np.array(img)
        corrupted_img = self.acvc_generator.corruption(img)
        return corrupted_img

@TRANSFORMS.register_module()
class ACVCCorruptionTransform(BaseTransform):
    def __init__(self):
        self.transforms = ACVCCorruption()

    def transform(self, results: dict) -> dict:
        img = results['img']
        corrupted_img = self.transforms(img)
