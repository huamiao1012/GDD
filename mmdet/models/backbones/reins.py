...
class Reins(nn.Module):
    ...
    def forward(
        self, feats: Tensor, layer: int, batch_first=False, has_cls_token=True, gt_masks = None,
    ) -> Tensor:
        if batch_first:
    ...
    def forward_delta_feat(self, feats: Tensor, tokens: Tensor, layers: int, gt_masks = None) -> Tensor:
        attn = torch.einsum("nbc,mc->nbm", feats, tokens)
        if self.use_softmax:
            attn = attn * (self.embed_dims**-0.5)
            if gt_masks is not None:
            # Expand mask to shape [B, N, 1] to broadcast on M
                mask = gt_masks.permute(1, 0).unsqueeze(-1).cuda()  # reverse: True → invalid
                attn = torch.einsum("nbc,mc->nbm", feats*mask, tokens)
                attn = attn * (self.embed_dims**-0.5)
